...
void Bot::Move() {
  ...
  if (appleFound) {
    ...
    if (appleX < snakeHeadX)
      priorityDirections.push_back({-1, 0}); // Влево
    if (appleX > snakeHeadX)
      priorityDirections.push_back({1, 0}); // Вправо
    if (appleY < snakeHeadY)
      priorityDirections.push_back({0, -1}); // Вверх
    if (appleY > snakeHeadY)
      priorityDirections.push_back({0, 1}); // Вниз
    ...
    for (auto dir : priorityDirections) {
      ...
      if (newX >= 0 && newX < _field.GetWidth() && newY >= 0 &&
          newY < _field.GetHeight() &&
