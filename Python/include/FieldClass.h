    ...
    if (!snake.GetBody().empty()) {
      if (&snake == &snakes.front()) {
        ...
        _field[snake.GetBody()[0].second][snake.GetBody()[0].first].SetType(
            CellType::SNAKE_HEAD_1); // Голова первой змеи
        ...
        _field[snake.GetBody().back().second][snake.GetBody().back().first]
            .SetType(CellType::SNAKE_TAIL_1); // Хвост первой змеи
        _field[snake.GetBody().back().second][snake.GetBody().back().first]
            .SetIsBusy(true);
      } else {
        _field[snake.GetBody()[0].second][snake.GetBody()[0].first].SetType(
            CellType::SNAKE_HEAD_2); // Голова остальных змей
        _field[snake.GetBody()[0].second][snake.GetBody()[0].first].SetIsBusy(
            true);
        for (size_t i = 1; i < snake.GetBody().size() - 1; ++i) {
          _field[snake.GetBody()[i].second][snake.GetBody()[i].first].SetType(
              CellType::SNAKE_BODY_2);
          _field[snake.GetBody()[i].second][snake.GetBody()[i].first].SetIsBusy(
              true);
        }
        _field[snake.GetBody().back().second][snake.GetBody().back().first]
            .SetType(CellType::SNAKE_TAIL_2); // Хвост остальных змей
        _field[snake.GetBody().back().second][snake.GetBody().back().first]
            .SetIsBusy(true);
      }
    }
...
void Field::UpdateMap(const Snake &snake1, const Snake &snake2) {
  ...
  if (!snake1.GetBody().empty()) {
    _field[snake1.GetBody()[0].second][snake1.GetBody()[0].first].SetType(
        CellType::SNAKE_HEAD_1);
    _field[snake1.GetBody()[0].second][snake1.GetBody()[0].first].SetIsBusy(
        true);
    for (size_t i = 1; i < snake1.GetBody().size() - 1; ++i) {
      _field[snake1.GetBody()[i].second][snake1.GetBody()[i].first].SetType(
          CellType::SNAKE_BODY_1);
      _field[snake1.GetBody()[i].second][snake1.GetBody()[i].first].SetIsBusy(
          true);
    }
    _field[snake1.GetBody().back().second][snake1.GetBody().back().first]
        .SetType(CellType::SNAKE_TAIL_1);
    _field[snake1.GetBody().back().second][snake1.GetBody().back().first]
        .SetIsBusy(true);
  }
  ...
  if (!snake2.GetBody().empty()) {
    _field[snake2.GetBody()[0].second][snake2.GetBody()[0].first].SetType(
        CellType::SNAKE_HEAD_2);
    _field[snake2.GetBody()[0].second][snake2.GetBody()[0].first].SetIsBusy(
        true);
    for (size_t i = 1; i < snake2.GetBody().size() - 1; ++i) {
      _field[snake2.GetBody()[i].second][snake2.GetBody()[i].first].SetType(
          CellType::SNAKE_BODY_2);
      _field[snake2.GetBody()[i].second][snake2.GetBody()[i].first].SetIsBusy(
          true);
    }
    _field[snake2.GetBody().back().second][snake2.GetBody().back().first]
        .SetType(CellType::SNAKE_TAIL_2);
    _field[snake2.GetBody().back().second][snake2.GetBody().back().first]
        .SetIsBusy(true);
  }
}